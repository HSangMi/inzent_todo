...
const actions = {
	...
	ADD_SUPER_TASK({ state, dispatch }, superTask) {
		...
		return api.project.addSuperTask(superTask).then(() => dispatch('FETCH_PROJECT', state.project.id));
	},
	ADD_SUB_TASK({ state, dispatch }, subTask) {
		console.log('actions.ADD_SUB_TASK : ');
		return api.project.addSubTask(subTask).then(() => dispatch('FETCH_PROJECT', state.project.id));
	},
	...
	ADD_CALENDAR_SUPER_TASKS(__, addSuper) {
		// 업무 대 추가
		return sohyun.schedule.addSuperTask(addSuper).then((result) => result).catch((err) => {
			console.log(err);
		});
	},
	ADD_CALENDAR_SUB_TASKS(__, addSub) {
		// 업무 소 추가
		return sohyun.schedule.addSubTask(addSub).then((result) => result).catch((err) => {
			console.log(err);
		});
	},
