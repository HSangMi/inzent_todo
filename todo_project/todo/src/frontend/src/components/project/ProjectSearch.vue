  ...
  <v-row>
      ...
      <v-card outlined height="800" class="overflow-y-auto mx-2">
        ...
        <div class="mx-2">
          <v-list class="project-filter">
            <v-subheader class="blue-grey lighten-4">검색 옵션</v-subheader>
            <v-radio-group v-model="option" @change="fatchData" color="primary" :mandatory="false">
          ...
          <v-list class="project-filter">
            <v-subheader class="blue-grey lighten-4">공개 범위</v-subheader>
            <v-radio-group v-model="range" @change="fatchData" color="primary" :mandatory="false">
    ...
    <v-col cols="10">
      <v-card outlined max-height="800" class="overflow-y-auto mx-2">
        <v-subheader class="blue-grey lighten-4" style="margin:8px">검색 결과</v-subheader>
        <!-- <v-card-title style="font-size:20px; font-weight:bold;">검색 결과</v-card-title> -->
        <v-card v-show="showProject"> 
      ...
      return [
        {
          ...
          { text: '공개 여부', align: 'center', value: 'usePublic', filter: this.rFilter },
      ]
    }, 
    taskHeaders() {
      return [
        ...
        { text: '업무경로', align: 'center', value: 'route', filterable: false },
        { text: '담당자', align: 'center', value: 'members', },
        { text: '기간', align: 'center', value: 'dueDate', },
        { text: '진행 상태', align: 'center', value: 'state', },
        { text: '공개 여부', align: 'center', value: 'usePublic', filter: this.rFilter },
  ...
  methods: {
    ...
    fatchData() {
     ...
     const srchData = {
      ...
      // option 
      if(this.option == 'opa') { // 전체
        this.showProject = true
        this.showTask = true
        this.itemCnt = 5
        this.projectHeaders[0].filterable = true     
        this.taskHeaders[0].filterable = true    
      } else if(this.option == 'opp') { // 프로젝트만
        this.showProject = true
        this.showTask = false
        this.itemCnt = 12
        this.projectHeaders[0].filterable = true
        this.taskHeaders[0].filterable = true     
      } else if(this.option == 'opt') { // 업무만    
        this.showProject = false
        this.showTask = true
        this.itemCnt = 12
        this.projectHeaders[0].filterable = true
        this.taskHeaders[0].filterable = true     
      } else if(this.option == 'opm') {  // 담당자       
        this.showProject = true
        this.showTask = true
        this.itemCnt = 5
