  ...
  <v-row>
    <v-col cols="2">
      <v-card outlined height="800" class="overflow-y-auto mx-2">
        ...
        ></v-text-field>
        <div class="mx-2">
          <v-subheader class="blue-grey lighten-4" style="margin-top:8px">사내 조직도</v-subheader>
          <v-treeview
            v-model="selection"
            :items="items"
            selectable
                      ...
                      <v-container>
                        <v-row> 
                          <v-col cols="12" sm="6" md="3">
                            <v-text-field outlined v-model="editedItem.id" label="사용자 ID" disabled></v-text-field>
                          </v-col>
                          <v-col cols="12" sm="6" md="3">
...
export default {
  ...
  methods: {  
    ...
    fatchUser() {
      if(this.userInfo.id == 'admin') {
        let depts = ["1100", "1200", "1300", "2100", "2300", "2210", "2220", "3200", "3300", "3110", "3120"]
        api.user.fatch_user(depts)
        .then((data) => {
          this.userList = data
        })
      } else {
    ...
    selectDept() {
      ...
      api.user.fatch_user(selectedDept)
      .then((data) => {
        this.userList = data
      })
    },
    ...
    deleteItem(item) {
      ...
      confirm(item.name + ' 사원을 삭제하시겠습니까??') && this.userList.splice(index, 1)
